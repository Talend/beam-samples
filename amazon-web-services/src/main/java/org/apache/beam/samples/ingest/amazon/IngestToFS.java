...
//  private interface Options extends PipelineOptions {
   private interface Options extends HadoopFileSystemOptions {
    ...
    @Description("Input Path") String getInput();
  ...
  public static void main(String[] args) {
    ...
    options.setOutput("/tmp/beam/ingesttofs");
    LOG.info(options.toString());
    System.out.println(options.toString());

    Pipeline pipeline = Pipeline.create(options);
//    pipeline.apply("ReadFromGDELTFile", TextIO.read().from(options.getInput()))
    pipeline.apply("ReadFromGDELTFile", TextIO.read().from(options.getHdfsConfiguration().get(0).get("fs.default.name")))
